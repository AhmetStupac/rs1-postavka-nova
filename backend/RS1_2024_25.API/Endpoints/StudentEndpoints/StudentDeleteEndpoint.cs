...
{
    [HttpDelete]
    public override async Task HandleAsync(StudentDeleteRequest req, CancellationToken cancellationToken = default)
    {
        var student = await db.Students.SingleOrDefaultAsync(x => x.ID == req.Id, cancellationToken);
        ...
        student.DeletedById = req.DeletedById;
